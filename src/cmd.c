...
void cli()
{
    ...
    // Read character from UART using getUart (which calls uart_getc internally)
    char c = getUart();  // getUart will block until a valid character is ready

    // If no character received (only space or invalid data), continue the loop
    if (c == 0)
    {
        return;
    }
